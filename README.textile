...
The current version of Redmine Xapian is **1.6.3** !https://api.travis-ci.org/picman/redmine_xapian.png(Build status)!:https://travis-ci.org/picman/redmine_xapian                                                    
...
Xapian gem is required and can be installed running bundle command in Redmine root directory. To install required xapian gem you might need to install uuid-dev package.
...
Now, go to the Redmine root directory and run bundle command to install required gems:
...
<pre>
@daily * * * * /usr/bin/xapian_indexer.rb
</pre>
